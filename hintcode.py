...
def giveHint(chosenCharacter):
    ...
    for char  in filteredchar:
        if len(filteredchar) == 1:
            description = description.replace(filteredchar[0], "_")
        elif len(filteredchar)==2:
            description = description.replace(filteredchar[0], "_")
            description = description.replace(filteredchar[1], "_")
        elif len(filteredchar)==3:
            description = description.replace(filteredchar[0], "_")   
            description = description.replace(filteredchar[1], "_")   
            description = description.replace(filteredchar[2], "_")

    hintvr = input('Wil je een hint?')

    if hintvr == 'ja':
        print(description)
    elif hintvr == 'nee':
        print('Er is geen hint nodig')
